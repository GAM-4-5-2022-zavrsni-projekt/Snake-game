from kivy.app import App
from kivy.uix.widget import Widget
from kivy.clock import Clock
from random import randint
from kivy.core.window import Window

class SnakePart(Widget):
    pass

class GameScreen(Widget):
    #stvaranje nove zmije
    step_size = 40
    movement_x = 0
    movement_y = 0
    snake_parts = []
    
    def new_game(self):
        #micanje zmije sa ekrana nakon završetka igre
        to_be_removed = []
        for child in self.children:
            if isinstance(child, SnakePart):
                to_be_removed.append(child)
        for child in to_be_removed:
            self.remove_widget(child)
        #izrada nove zmije na početku igre               
        self.snake_parts = []
        head = SnakePart()
        head.pos = (0,0)
        self.snake_parts.append(head)
        self.add_widget(head)

    def on_touch_up(self, touch):
        dx = touch.x - touch.opos [0]
        dy = touch.y - touch.opos [1]
        
        #pomicanje desno i lijevo
        if abs(dx) > abs(dy):
            self. movement_y = 0
            if dx > 0:
                self.movement_x = self.step_size
            else:
                self.movement_x = - self.step_size
                
        #pomicanje gore i dolje
        else:
            self.movement_x = 0
            if dy > 0:
                self.movement_y = self.step_size
            else:
                self.movement_y = - self.step_size
        
    def collides_widget(self,wid1, wid2):
    #provjerava sudare widgeta
        if wid1.right <= wid2.x:
            return False
        if wid1.x >= wid2.right:
            return False
        if wid1.top <= wid2.y:
            return False
        if wid1.y >= wid2.top:
            return False
        return True

    def next_frame(self, *args):
        

        head = self.snake_parts[0]
        food = self.ids.food

        #dijelovi nakon glave
        last_x = self.snake_parts[-1].x
        last_y = self.snake_parts[-1].y

        #pomicanje zmije
        for i, part in enumerate(self.snake_parts):
            if i == 0:
                continue
            part.new_y = self.snake_parts[i-1].y
            part.new_x = self.snake_parts[i-1].x
        for part in self.snake_parts[1:]:
            part.y = part.new_y
            part.x = part.new_x
        
       #pomicanje glave
        head.x += self.movement_x
        head.y += self.movement_y

        #sudari s hranom
        if self.collides_widget(head, food):
            #mijenja poziciju hrane
            food.x =  randint (0, Window.width-food.width)
            food.y =  randint (0, Window.height-food.height)
            #dodaje dio zmiji
            new_part = SnakePart()
            new_part.x = last_x
            new_part.y = last_y
            self.snake_parts.append(new_part)
            self.add_widget(new_part)
            
        #sudar zmije sa samom sobom
        for part in self.snake_parts[1:]:
            if self.collides_widget(part, head):
                self.new_game()
                
        #sudar sa granicama ekrana
        if not self.collides_widget(self, head):
            self.new_game()
    
    
class MainApp(App):
#frame se refresha svakih 5 sekundi
    def on_start(self):
        self.root.new_game()
        Clock.schedule_interval(self.root.next_frame, .5)
    pass
MainApp().run()
